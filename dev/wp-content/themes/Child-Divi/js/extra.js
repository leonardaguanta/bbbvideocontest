(function ( $ ) {
	...
	var extra = {
		...
		init: function () {
			...
			this.verifyUploadSize();
                ...
                adminDeletePending : function(){
                	...
                	$('.toReject a').on('click',function(e){
                        ...
                        mscConfirm("Are you sure you want to reject this video?",function(){

                            mscAlert("Video Rejected",function(){
                                location.href = href;
                            });
                        });
        	...
        	$('#modal-'+id+' .highlow').html(result['highlow']);
        	/*$('#modal-'+id+' .average-duration.').html(result['avgDuration']);*/
        	console.log(result);
        	function drawChart() {
        		//var data = google.visualization.arrayToDataTable([['days', 'Views' ],result['dataView']]);
        		//"["W", 0],["TH", 7],["F", 1],["S", 0],["S", 0],["M", 0],["T", 0]"

        		var data = google.visualization.arrayToDataTable([['days', 'Views' ],["W", 0],["TH", 7],["F", 1],["S", 0],["S", 0],["M", 0],["T", 0]]);
        		//var data = google.visualization.arrayToDataTable([['Day','TurbTemp_F'], ['1',45.68], ['2',45.68], ['3',45.86], ['4',45.86], ['5',45.86], ['6',45.86], ['7',45.86]]);
                console.log(data);
                var options = {
        ...
        error: function () {
        	alert("error");
                ...
                adminDeleteAccepted : function(){
                	...
                	$('.approved-pending').on('click',function(e){
                      ...
                      mscConfirm("Are you sure you want to deactivate this video?",function(){
                          $.ajax({
                            type: 'POST',
                            url: ajax_object.ajax_url,
                            data: {
         ...
         mscAlert("Video Deactivated",function(){console.log(result);
             location.reload();});
			...
			//}	
                ...
                adminDeleteUser : function(){
                  ...
                  $('.bbbAdminDeleteUser').on('click',function(e){
                    ...
                    mscConfirm("Are you sure you want to delete this user?",function(){
                        $.ajax({
                            url: ajax_object.ajax_url,
                            type : 'GET',
                            dataType : 'json',
                          ...
                          success :function(respond){
                            ...
                            mscAlert("User Deleted",function(){
                ...
                adminDeleteSchool : function(){

                  $('.sortStatsButton.delete-school').on('click',function(e){
                    e.preventDefault();
                    var current_el = $(this);
					console.log(current_el.attr('data-id'));
					
                    mscConfirm("Are you sure you want to delete this school?",function(){
                        $.ajax({
                            url: ajax_object.ajax_url,
                            type : 'GET',
                            dataType : 'json',
                          ...
                          success :function(respond){
                            ...
                            mscAlert("School Deleted",function(){


                            });
                            console.log(respond);
                            if (respond.result){
                               current_el.closest('.school-admin').remove();
                            } 
							  location.reload();
