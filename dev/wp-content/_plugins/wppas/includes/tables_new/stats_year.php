<?php
class Pro_Ad_Stats_Year_List_Table extends WP_List_Table {
	...
	{	
        ...
        switch($column_name){
            case 'count':
				return $item->hits.' '.$pro_ads_statistics->stat_types( $item->type );
            default:
				return '<a href="javascript:void(0)" class="time_frame_btn" rid="3" type="'.$item->type.'" year="'.date('Y', $item->date).'" month="'.date('m', $item->date).'" day="'.date('d', $item->date).'">'.date_i18n('F', $item->date).'</a>';
	...
	*/
    ...
    function prepare_items( $arr = array() ) {
		...
		// @used till v4.5.3
		//$data = $wpdb->get_results('SELECT *, COUNT(id) as count_nr, MONTH(FROM_UNIXTIME(date)) as themonth FROM '.$wpdb->prefix.'pro_ad_system_stats '.$where_status.' GROUP BY themonth');
		$data = $wpdb->get_results('SELECT *, SUM(hits) as hits, COUNT(id) as count_nr, MONTH(FROM_UNIXTIME(date)) as themonth FROM '.$wpdb->prefix.'wpproads_user_stats '.$where_status.' GROUP BY themonth');
