...
class Pro_Ad_Stats_Day_List_Table extends WP_List_Table {
	...
	{	
        ...
        switch($column_name){
			...
			case 'browser':
				return !empty($item->browser) ? '<img src="'.WP_ADS_URL.'images/browser/'.$item->browser.'.png" alt="'.$item->browser.'" />' : __('n/a','wpproads');
			case 'platform':
				return !empty($item->platform) ? '<img src="'.WP_ADS_URL.'images/platform/'.$item->platform.'.png" alt="'.$item->platform.'" />' : __('n/a','wpproads');
			case 'user_info':
				$html = '';
				...
				return $html;
            default:
				return date_i18n('l, d - G:i', $item->time);
	...
	*/
     ...
     **************************************************************************/
    function prepare_items( $arr = array() ) {
        global $wpdb, $pro_ads_main, $pro_ads_statistics, $pro_ads_advertisers; //This is used only if making any database queries
		...
		{
			$data = $pro_ads_statistics->load_statistics( 'WHERE date = '.mktime(0,0,0, $res['month'], $res['day'], $res['year']).' AND type = "'.$res['type']['slug'].'" '.$group_query );
		}
		else
		{
			$advertiser = $pro_ads_advertisers->get_advertisers( array('meta_key' => '_proad_advertiser_wpuser', 'meta_value' => get_current_user_id()) );
			$data = $pro_ads_statistics->load_statistics( 'WHERE date = '.mktime(0,0,0, $res['month'], $res['day'], $res['year']).' AND type = "'.$res['type']['slug'].'" AND advertiser_id = '.$advertiser[0]->ID.$group_query );
		}
	 ...
	 * @access public
	 */
	function ajax_response( $arr = array() ) 
	{
	...
	function ajax_script() 
	{
        ...
        <script type="text/javascript">
			jQuery(document).ready(function($) {
				...
				$("body").on("click", "#day-stats-filter .tablenav-pages a, #day-stats-filter .manage-column.sortable a, #day-stats-filter .manage-column.sorted a", function(e){
					...
					//alert(query)
					//var paged = __query( query, 'paged' ) || '1';
					var data = {
						paged: __query( query, 'paged' ) || '1',
						order: __query( query, 'order' ) || 'asc',
						orderby: __query( query, 'orderby' ) || 'title',
						day: $('#day-stats-filter').attr('day'),
						month: $('#day-stats-filter').attr('month'),
						year: $('#day-stats-filter').attr('year'),
						type: $('#day-stats-filter').attr('type'),
					};
				...
				// Page number input
				$("body").on('keyup', '#day-stats-filter input[name=paged]', function(e) {
				//$('#day-stats-filter input[name=paged]').on('keyup', function(e) {
					...
					var data = {
						paged: parseInt( $('#day-stats-filter input[name=paged]').val() ) || '1',
						order: $('#day-stats-filter input[name=order]').val() || 'asc',
						orderby: $('#day-stats-filter input[name=orderby]').val() || 'title'
					};
					   ...
					   data: $.extend(
							{
								...
								action: '_ajax_fetch_custom_list_day',
							},
							data
					   ),
