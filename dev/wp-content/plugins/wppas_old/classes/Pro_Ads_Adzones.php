...
class Pro_Ads_Adzones {	
	...
	*/
	public function get_adzone_data( $adzone_id, $args = array() ) 
	{
		...
		if( !$adzone_center )
		{
			...
			{
				$size_str = !empty($size) ? 'max-width:'.$size[0].'px; max-height:'.$size[1].'px;' : ' width:100%; height:auto; display:inline-block; '; // width:auto; (till v4.6.5)
			}
			else
			{
	...
	*/
	public function wpproads_adzone_container( $adzone_id, $arr, $banners = array() )
	{
		...
		$html.= '<div '.$arr['adzone_options']['fixed_class'].' class="paszone_container paszone-container-'.$adzone_id.' '.$arr['atts']['screen'].' '.$arr['atts']['class'].' '.$arr['atts']['background_pattern'].'" id="paszonecont_'.$adzone_id.'" style="overflow:hidden; '.$arr['adzone_options']['container_max_width'].' '.$arr['adzone_options']['center_css'].' '.$arr['adzone_options']['align_css'].' '.$arr['adzone_options']['paszone_container_style'].'" '.$html_attributes.'>';
			...
			$html.= '<div class="wppaszone proadszone-'.$adzone_id.' '.$arr['atts']['screen'].'" id="'.$adzone_id.'" style="overflow:hidden; '.$arr['size_str'].' '.$arr['adzone_options']['center_css'].' ">'; //'.$arr['adzone_options']['fixed_class'].'
					...
					$html.= 'jQuery(function($){';
						...
						$html.= 'var slider_'.$arr['rotate_class'].' = $(".'.$arr['rotate_class'].'").bxSlider({ ';
							...
							$html.= 'onSliderLoad: function(currentIndex){ ';
								...
								if( $arr['rotate_ajax'] )
								{
									$html.= 'var next_banner = $(".'.$arr['rotate_class'].'>.pasli").eq(currentIndex+2).attr("bid");';
	...
	*/
	...
	{	
		global $pro_ads_main, $pro_ads_codex, $pro_ads_templates, $pro_ads_banners, $pro_ads_statistics, $pro_ads_multisite, $pro_ads_responsive, $pro_ads_bs_templates, $pro_ads_bs_woo_templates, $pro_geo_targeting_main, $pro_ads_shortcodes;
		...
		if( isset( $id ) )
		{
			...
			$html = '';
			$device = $pro_ads_responsive->get_device_type();
			$hide_for_device = get_post_meta( $id, '_adzone_hide_for_device'.$device['prefix'], true );
			...
			*/
			...
			/*
			 * Hide ads for specific device?
			 */
			$show = $hide_for_device ? 0 : $show;
			/*
			 * Filter to show/hide adzones
			*/
			$show = apply_filters( 'wp_pro_ads_show_adzone', $show, $id );
			...
			if( $show )
				...
				{   
					...
					{       
						...
						{
							// check if campaign is active
							$campaign_status = $pro_ads_codex->wpproads_campaign_is_active( $banner->ID );
							...
							if( $campaign_status == 1 )
							{
								...
								{ 
									...
									// Load first banner if ajax load is selected.
										...
										$html.= 'jQuery(window).load(function(){';
											$html.= 'jQuery(function($){';
												$html.= '$.ajax({';
												   ...
												   $html.= 'success: function( msg ){';
														$html.= '$(".pasli-'.$banner->ID.'").html( msg ); $(".pasli-'.$banner->ID.'").removeClass("placeholder");'; // $(".pasli-'.$banner->ID.'").removeAttr("style");
												   $html.= '}';
												$html.= '});';
												$html.= '});';
											$html.= '});';
								...
								else
								{
									...
									if(!$atts['no_impression']) {
									   $pro_ads_statistics->save_impression( $banner->ID, $id, $ref_url );
									}
								}
								$active_banners++;
							}
					...
					if (empty($banners)){
                                            $default_banner = get_post_meta($id,'_adzone_default_url', true);
                                            if($default_banner){
                                              return sprintf('<a href="%s"><img src="%s" title="%s"></a>',esc_url(home_url()),esc_attr($default_banner),esc_attr(get_the_title($id)));
                                            }else {
                                              return false;
                                            }
					}
                                        ...
                                        foreach($banners as $banner){
                                        	if ( get_post_meta($banner->ID,'_banner_is_html5', true) ) {
                                              //		$bhtml = ''; 
                                              		//$bhtml .= do_shortcode(get_post_meta($banner->ID,'_banner_html', true));
                                        	}	 
                                        }	
	...
	*/
	...
	{
		global $pro_ads_main, $pro_ads_codex, $pro_ads_banners, $pro_ads_statistics, $pro_ads_templates, $pro_ads_responsive, $pro_ads_bs_templates, $pro_ads_bs_woo_templates, $pro_geo_targeting_main, $pro_ads_multisite, $pro_ads_shortcodes;
			...
			*/
			...
			if( $show )
			{
				...
				*/
				...
				{
					$html.= '<div '.$arr['adzone_options']['fixed_class'].' class="cs wppaszone proadszone-'.$id.' wpproadgrid '.$class.'" style="overflow:hidden; '.$arr['adzone_options']['center_css'].' '.$arr['adzone_options']['align_css'].'">';
						...
						$html.= '<div style="display:inline-block;">';
							...
							for($i = 0; $i < $grid_limit; $i++ )
							{
								...
								if( !empty($banners[$i]) && $campaign_status == 1 )
								{
									...
									/*if( !$responsive )
									{
										$html.= '<div class="grid_container grid_container_'.$id.'" style="float:left; '.$size_str.' margin:'.$arr['margin'].'px;">';
											$html.= $pro_ads_banners->get_banner_item( array('id' => $banners[$i]->ID, 'aid' => $id, 'force_size' => $arr['adzone_size']));
										$html.= '</div>';
									}
									else
									{
										$html.= '<div class="grid_container  grid_container_'.$id.'" style="float:left; '.$size_str.' margin:'.$arr['margin'].'px;">';
											$html.= $pro_ads_banners->get_banner_item( array('id' => $banners[$i]->ID, 'aid' => $id));
										$html.= '</div>';
									}*/
	...
	*/
	...
	{	
		...
		if( !empty($adzone_id))
		{
			$show = 1;
			$device = $pro_ads_responsive->get_device_type();
			$hide_for_device = get_post_meta( $adzone_id, '_adzone_hide_for_device'.$device['prefix'], true );
			...
			*/
			if( $pro_ads_main->pro_geo_targeting_is_active() )
			{
				$show = $pro_geo_targeting_main->show_content( $adzone_id );
			}
			/*
			 *_______________________________________________________________________________________________________________
			*/
					...
					*/
					...
					if( $banners )
					{
						...
						$html.= '<script type="text/javascript">/* <![CDATA[ */';
							...
							$html.= '"link_full":"'.addslashes( $pro_ads_banners->pro_ads_create_banner_link(array('banner_id' => $banners[0]->ID, 'adzone_id' => $adzone_id)) ).'",';
		...
		if( !empty($adzone_id))
		{
			...
			*/
			...
			if( $show )
			{
				...
				$html.= '<div id="corncurl-bg"><div class="corncurl-content">'.$this->display_adzone( $adzone_id, $atts ).'</div></div>';
