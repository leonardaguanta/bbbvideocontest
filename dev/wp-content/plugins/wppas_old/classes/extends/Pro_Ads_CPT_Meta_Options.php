...
?>
                    ...
                    <tr valign="top">
                        ...
                        <td>
                            ...
                            <input type="hidden" name="end_date" id="end_date" <?php echo !empty($end_date) ? date_i18n('m.d.Y', $end_date) : ''; ?> />
...
<?php
    ...
    function wp_pro_ads_banners_meta_options_save_postdata( $post_id ) 
    {
        ...
        foreach( $device_sizes as $i => $device_size )
        {
            ...
            $banner_is_html5   = $_POST['banner_is_html5'.$device_size['prefix']] ? 1 : 0;
...
<?php 
            if ( empty(${'banner_html'.$device_size['prefix']}) ){
              echo $pro_ads_banners->get_banner_preview( $post->ID, $device_size['prefix'], 1, 'url' );
            } else {
                $video_id = filter_var( ${'banner_html'.$device_size['prefix']}, FILTER_SANITIZE_NUMBER_INT);
?>
                    <video width="320" height="240" controls>
                      <source src="<?php echo get_post_meta( intval($video_id), 'fp5-mp4-video', true ); ?>" type="video/mp4">
                    </video>
              <?php
                echo shortcode_exists('flowplayer');
            }
?>
                                                            </div>
                                                        </td>
                                                    ...
                                                    <tr valign="top">
                                                        <th scope="row">
                                                            <?php _e( "Video Ad", 'wpproads' ); ?>
                                                            <span class="description"><?php _e('Upload/Select a video.', 'wpproads'); ?></span><br />
                                                        </th>
                                                        <td>  <input type="text" size="40" id="banner_video_title<?php echo $device_size['prefix']; ?>" name="banner_video_title<?php echo $device_size['prefix']; ?>" value="<?php echo !empty( ${'banner_video_title'.$device_size['prefix']} ) ? ${'banner_video_title'.$device_size['prefix']} : ''; ?>" placeholder="<?php _e('Video title', 'wpproads'); ?>" />
                                                              <br /> 
                                                              <br />  
<?php wp_nonce_field('video_ad_upload', 'video_ad_nonce'); ?>
                                                             <input type="text" size="40" id="banner_video<?php echo $device_size['prefix']; ?>" name="banner_video<?php echo $device_size['prefix']; ?>"  placeholder="<?php _e('Video url', 'wpproads'); ?>" />
                                                                <input class="upload_video_button<?php echo $device_size['prefix']; ?> button" type="button" value="<?php _e('Upload Video', 'wpproads'); ?>" />
<br />
<br />
<?php
            //flowplayer5 CPT
            ...
            $video_ad_cpt = get_posts($video_ad_args);?>
             
             <select class="video_ad_shortcode banner_html"  name="banner_html<?php echo $device_size['prefix']; ?>" >
<?php foreach($video_ad_cpt as $video):?>
     <?php $shaneFix = $video->ID; ?>                                                  
     <option value="<?php echo "[flowplayer id='$shaneFix']"; ?>" <?php selected(${'banner_html'.$device_size['prefix']}, '[flowplayer id="'.$video->ID.'"]');?>><?php echo esc_html($video->post_title); ?></option>
 
<?php endforeach; ?>
</select>   
<span class="description">Select which video ad and will generate a shortcode</span>
                                                         <!--<textarea name="banner_html<?php echo $device_size['prefix']; ?>" style="width:100%; height:200px;"><?php echo !empty( ${'banner_html'.$device_size['prefix']} ) ? ${'banner_html'.$device_size['prefix']} : ''; ?></textarea>-->
                                                            <br /><br />
                                                            <div>
 <input id="banner_is_html5_btn" name="banner_is_html5<?php echo $device_size['prefix']; ?>" type="hidden" value="true" />
