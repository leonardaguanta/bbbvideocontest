...
//            wp_enqueue_style($this->plugin_slug . '-live-validation', plugins_url('assets/css/validation.css', __FILE__), array(), self::VERSION);
        ...
        public function get_validation_string($validation, $field) {
            ...
            foreach ($validation as $key => $config) {
                ...
                if (!in_array($this->getInputtype($field), array('checkbox', 'radio', "name", "date", "email", "address", "time"))) {
                    ...
                    echo "all_validations['" . $field['id'] . "'] =  f" . $field['id'] . ";";
                    $js_var = ' f' . $field['id'];
                }
                ...
                foreach ($config as $eachValidation) {
                    ...
                    if ($incr == 0 && in_array($this->getInputtype($field), array('name', "date", "email", "address", "time"))):
                        ...
                        echo "all_validations['" . $eachValidation['field_id'] . "'] =  f" . $eachValidation['field_id'] . ";";
                            ...
                            switch ($eachValidation['ftype']):
                                ...
                                case "text":
                                    ...
                                    if (rgar($field, "inputMaskValue") == "" && (rgar($field, "phoneFormat") !== "standard")) {
                                        ...
                                        echo '' . $js_var . '.add(Validate.Presence,{ failureMessage: "' . $get_default_error . '",validMessage: " " ,jqObj: jqr,mask:"' . $this->get_masking_presence_validator("(999) 999-9999") . '",livevalidkey:"' . $current_key . '" });';
                                    } else {
                                        echo '' . $js_var . '.add(Validate.Presence,{ failureMessage: "' . $get_default_error . '",validMessage: " " ,jqObj: jqr,mask:"' . $this->get_masking_presence_validator(rgar($field, "inputMaskValue")) . '",livevalidkey:"' . $current_key . '" });';
                                    }
                                    break;
                                ...
                                case "radio":
                                    ...
                                    for ($i = 0; $i <= (count($field['choices']) - 1); $i++) {
                                        ...
                                        echo "all_validations['" . $field['id'] . "'] = f" . $field['id'] . "_" . $i . ";";
                                        echo 'f' . $field['id'] . '_' . $i . '.add(Validate.AcceptanceRadio,{ failureMessage: "' . $get_default_error . '" ,validMessage: " " ,jqObj: jqr,name_field:"input_' . $field['id'] . '" });';
                                    }
                                    break;
                                ...
                                case "checkbox":
                                    ...
                                    if (rgar($field, "displayAllCategories") == 1) {
                                        ...
                                        for ($i = 1; $i <= (count($terms)); $i++) {
                                            ...
                                            echo ' var f' . $field['id'] . '_' . $i . ' = new LiveValidation("choice_' . $field['formId'] . '_' . $field['id'] . '_' . $i . '",jqr.extend({validMessage: " ", jqObj: jqr ,fieldType:"' . $field['type'] . '"},additional_data ));';
                                            echo "all_validations['" . $field['id'] . "'] = f" . $field['id'] . "_" . $i . ";";
                                            echo 'f' . $field['id'] . '_' . $i . '.add(Validate.AcceptanceCheckbox,{ failureMessage: "' . $get_default_error . '" ,validMessage: " " ,jqObj: jqr,field_name:"input_' . $field['id'] . '",field_id:"' . $field['id'] . '"});';
                                        }
                                    } else {
        ...
        public function lv_apply_validations_to_form($form) {
            ...
            echo '<script type="text/javascript">var mygf_form_ID = ' . $form['id'] . ';</script>';

            $ajax = "no";
            if ($this->has_ajax):
                $ajax = "yes";
            endif;
            echo '<script type="text/javascript">var lv_gf_is_ajax = "' . ($ajax) . '"; var all_validations = {};</script>';
  ...
  console.error("Error Applying validations!!");
        ...
        public function get_pattern_for_js($format) {
            ...
            //matching if format already has any modifieres?
            preg_match($pattern, $format, $matches);

            if ($matches && is_array($matches) && count($matches) > 0) {


                return preg_replace($pattern, "/i", $format);
            } else {
