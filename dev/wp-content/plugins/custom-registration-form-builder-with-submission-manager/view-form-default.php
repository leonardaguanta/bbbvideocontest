...
wp_enqueue_style( 'crf-style-default', plugin_dir_url(__FILE__) . 'css/crf-style-default.css');
        ...
        <div class="formtable">
          ...
          <div class="crf_input crf_select <?php if($row1->Require==1)echo 'crf_select_required';?>">
            <select class="regular-text <?php echo $row1->Class;?>" id="<?php echo $key;?>" name="<?php echo $key;?>" <?php if($row1->Readonly==1)echo 'disabled';?>>
              <?php
		if($row1->Custom_Post_Type) {
			$args = array(
                        	'post_type' => $row1->Custom_Post_Type,
                        	'posts_per_page' => 100,
                        	'public' => true,
                        	'post_status' => 'publish'
                	);
			...
			?>
		<option value="<?php echo $post->ID;?>" <?php if($post->ID==$value)echo 'selected';?>><?php echo $post->post_title;?></option>
			<?php
			}
		} else {
    ...
    function validete_email() //AJAX email validation
        {
            jQuery.ajax({
                type: "POST",
                url: '<?php admin_url('admin-ajax.php'); ?>?action=crf_ajaxcalls&cookie=encodeURIComponent(document.cookie)&function=validateEmail&email=' + jQuery("#user_email").val(),
                success: function (serverResponse) {
                    if (serverResponse == "true") {
                        email = false;
