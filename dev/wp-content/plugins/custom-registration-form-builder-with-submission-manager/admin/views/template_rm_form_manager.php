...
<div class="rmagic rmbasic">
    ...
    <li data-id="rmbar" data-options="tipLocation:bottom">You will see this flat white box on top of different sections inside RegistrationMagic. We call it operations bar. It contains...</li>
        ...
        <li data-id="rm-ob-duplicate">This allows you to duplicate one or multiple forms. Form's configuration and fields are also duplicated. Note: This does not duplicates conditional logic applied to the form.</li>
        <li data-id="rm-ob-delete">This allows you to delete one or multiple forms. All associated form data is deleted.</li>
        <li data-id="rm-ob-export">This allows you to export all your forms and associated data in a single XML file. Handy if you are reinstalling your site, moving forms to another site or simply backing up your hard work. Note: This does not exports conditional logic applied to the form.</li>
        ...
        <li data-id="rm-card-area">All form cards are displayed as grid, starting from here. You may not need to create more than one registrations form, but it's totally up to you. RegistrationMagic gives a playground to experiment and play to find the best combination for your site. First card slots is reserved for <b>Login Form</b></li>
        ...
        <li data-class="rm_formcard_menu_icon" data-options="tipAdjustmentX:-25;tipAdjustmentY:-2">Clicking this will open a popup menu allowing you direct access to different sections of this form.</li>
        ...
        <li data-class="rm_def_star_tour" data-options="tipAdjustmentX:-24;tipAdjustmentY:-5">This little star allows you to mark a form as your default registration form.</li>
        ...
        <div class="operationsbar" id="rmbar">
            ...
            <div class="nav" id="rm-ob-nav">
                <ul>
                    ...
                    <?php $localized_str_exportall = RM_UI_Strings::get('LABEL_EXPORT')." (".RM_UI_Strings::get('LABEL_ALL').")"; $localized_str_exportselected = RM_UI_Strings::get('LABEL_EXPORT'); ?>
            ...
            foreach ($data->data as $index=>$entry)
            {
                ...
                <?php endif; ?>
                    <div class="cardtitle rm-title-tour">
                        <input class="rm_checkbox rm-checkbox-tour" type="checkbox"  onclick="rm_on_form_selection_change()" name="rm_selected_forms[]" value="<?php echo $entry->form_id; ?>"><span class="rm_form_name" ><?php echo htmlentities(stripslashes($entry->form_name)); ?></span>
                    </div>
                    ...
                    <div class="rm-form-links">
                                <div class="rm-form-row rm-formcard-dashboard"><a class="rm-form-settings" href="admin.php?page=rm_form_sett_manage&rm_form_id=<?php echo $entry->form_id; ?>">Dashboard</a>
                                </div>  
                                <div class="rm-form-row rm-formcard-setup"><a class="rm-form-fields" href="admin.php?page=rm_field_manage&rm_form_id=<?php echo $entry->form_id; ?>">Fields</a></div> 
                     </div>
...
?>
    ...
    <div id="rm_add_new_form_popup" class="rm-modal-view" <?php echo $new_form_pop_up_style;?>>
        ...
        <div class="rm-modal-wrap rm-create-new-from">
            <div class="rm-modal-titlebar rm-new-form-popup-header">
                <div class="rm-modal-title">
                    <?php echo RM_UI_Strings::get('LABEL_ADD_NEW');?>
                </div>
   ...
   function rm_start_joyride(){
    ...
    function  rm_on_form_selection_change() {    
        ...
        if(selected_forms.length > 0) {   
            jQuery("#rm-ob-export a").text( jQuery("#rm-ob-export").data("rmlocalstrselected") + ' (' + selected_forms.length +')');
            jQuery("#rm-ob-delete").removeClass("rm_deactivated");   
            jQuery("#rm-ob-duplicate").removeClass("rm_deactivated");
        } else {
            jQuery("#rm-ob-export a").text(jQuery("#rm-ob-export").data("rmlocalstrall"));
...
jQuery("#rm_rateit_banner").bind('rated', function (event, value) { 
    ...
    jQuery(document).ready(function () {
        jQuery('.rm-modal-close, .rm-modal-overlay').click(function () {
            jQuery(this).closest('.rm-modal-view').hide();
        });
    });
        ...
        jQuery.post(ajaxurl, data, function (response) {
            if (response == 0)
            {
               _getEl("progressBar").value = Math.round(100);
                _getEl("status").innerHTML = 'Import Successfully Completed.';
                setTimeout(function(){
    ...
    function start_import(){
        ...
        jQuery.post(ajaxurl, data, function (response) {
            if (response == 0)
            {
                ...
                jQuery("#rm_import_errors").html('');
                jQuery("#rm_import_errors").append("<div class='rm_import_error'>Invalid RegistrationMagic template file. Please upload valid template file with XML extension.</div>");
    ...
    function check_file_extension(obj){
        ...
        if(file && file.type!="text/xml"){
            jQuery("#rm_import_errors").html("<div class='rm_import_error'>Invalid RegistrationMagic template file. Please upload valid template file with XML extension.");
            obj.value='';
        }
    }
    ...
    function uploadFile() {
      ...
      if(!file){
           jQuery("#rm_import_errors").html("<div class='rm_import_error'>Please select a file.</div>");
           return;
      }
      jQuery("#rm_import_errors").html('');
    ...
    function progressHandler(event) {
      ...
      _getEl("status").innerHTML = 'File Upload is in Progress....';
    }

    function completeHandler(event) {
       var percent = 50;
      _getEl("progressBar").value = Math.round(percent);
      _getEl("status").innerHTML = 'Form Import is in Progress....';
      start_import();
    }
