...
class Pro_Ads_Templates {	
        ...
        <script type="text/javascript">
            jQuery(document).ready(function($){
				...
				$('#<?php echo $form_id; ?> #create_shortcode').on('click', function(){
					...
					if( $('#<?php echo $form_id; ?> #adzone_flyin_position').val() == 1 ){
	...
	*/
	...
	{
		global $pro_ads_adzones, $pro_ads_codex, $pro_ads_responsive;
		
		$adzone_data = $pro_ads_adzones->get_adzone_data($arr['adzone_id']);
		...
		$linked_banner_ids = get_post_meta( $arr['adzone_id'], '_linked_banners', true );
		$banners = $pro_ads_codex->wpproads_load_adzone_banners( $adzone_data );
		$cookie = !empty($arr['atts']['cookie']) ? $arr['atts']['cookie'] : 0;
		
		$html = '';
		
		if( empty($linked_banner_ids) && $adzone_data['adzone_hide_empty'] || empty($banners) )
		{
		...
		else
		{
			...
			$hide_for_device = get_post_meta( $arr['adzone_id'], '_adzone_hide_for_device'.$device['prefix'], true );
		
			if( !$hide_for_device )
			{
				$html.= '<div id="backgroundPasPopup" style="'.$popup_bg.$popup_opacity.'"></div>';
				$html.= '<div class="PasPopupCont" style="visibility: hidden;">';
				...
				$html.= empty($exit_popup) ? '<script>jQuery(document).ready(function($){loadPASPopup( 0, "success", "'.admin_url('admin-ajax.php').'", '.$cookie.' );});</script>' : '';
					...
					$html.= '<script type="text/javascript">';
						$html.= 'jQuery(document).ready(function($){';
							$html.= '$(document).mouseleave(function(){';
								$html.= 'loadPASPopup( 0, "success", "'.admin_url('admin-ajax.php').'", '.$cookie.' );';
							$html.= '});';
						$html.= '});';
					$html.= '</script>';
				}
	...
	*/
	public function pro_ad_fly_in( $arr = array() )
	{
		...
		$delay = !empty($arr['delay']) ? $arr['delay'] : 3000;
